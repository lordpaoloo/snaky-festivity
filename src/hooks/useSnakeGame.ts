...
const FOODS_PER_LEVEL = 5; // Adjusted to gain 2 levels per food
...
export const useSnakeGame = () => {
  ...
  const [level, setLevel] = useState<number>(0); // Start at level 0
    ...
    setSnake(prevSnake => {
      ...
      if (head.x === food.x && head.y === food.y) {
        ...
        setFoodEaten(prev => {
          ...
          const newLevel = (newFoodEaten * 2); // Gain 2 levels per food, starting from 0
          setLevel(newLevel);
